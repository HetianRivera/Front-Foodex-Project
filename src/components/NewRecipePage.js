...
export function NewRecipePage({ onCancel, onSave, user }) {
  ...
  const RELAX = String(process.env.REACT_APP_RELAX_RECIPE_VALIDATION || process.env.REACT_APP_OFFLINE || process.env.REACT_APP_USE_MOCK || 'false').toLowerCase() === 'true';
  ...
  const handleSave = async () => {
    ...
    if (!RELAX) {
      ...
      if (!procesos.some(p => p.titulo.trim() && p.descripcion.trim() && p.tiempoEstimado))
        newErrors.procesos = 'Debes completar al menos una etapa con título, tiempo y descripción';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:text-slate-100 transition-colors duration-500 ease-in-out">
      <FullScreenLoader show={isSaving} />      <DashboardHeader user={user}>
